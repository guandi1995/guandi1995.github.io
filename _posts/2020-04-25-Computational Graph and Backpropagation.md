...
The logistic regression example shown above is under the assumption that the dimension feature are two dimensions, that is $$x=\begin{bmatrix}x_1&x_2\\\end{bmatrix}^T$$. Consequently, the formulas we have above are $$\dfrac{dL}{dw_1} = (\hat{y}-y)x_1$$, $$\dfrac{dL}{dw_2}= (\hat{y}-y)x_2$$. Suppose the feature dimensions are n-dimension, then the gradient formulas for $$\dfrac{dL}{dw_1},...,\dfrac{dL}{dw_n}$$ is that $$\dfrac{dL}{dw_1} = (\hat{y}-y)x_1,...$$, $$\dfrac{dL}{dw_n} = (\hat{y}-y)x_n$$. To vectorize it, we have $$\dfrac{dL}{dw} = (\hat{y}-y)x$$ where $$x=\begin{bmatrix}x_1&..&x_n\\\end{bmatrix}^T$$ and $$w=\begin{bmatrix}x_1&..&x_n\\\end{bmatrix}^T$$. 
