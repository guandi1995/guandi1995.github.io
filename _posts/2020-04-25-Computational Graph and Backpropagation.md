...
First of all, the thing we need to notice that the logistic regression example for a single sample shown above is under the assumption that the dimension feature are two dimensions, that is $$x=\begin{bmatrix}x_1&x_2\\\end{bmatrix}^T$$. Consequently, the formulas we have above are $$\dfrac{dL}{dw_1} = (\hat{y}-y)x_1$$, $$\dfrac{dL}{dw_2}= (\hat{y}-y)x_2$$. Suppose the feature dimensions are n-dimension, then the gradient formulas for $$\dfrac{dL}{dw_i}$$ where $$i\in{1,..,n}$$ are $$\dfrac{dL}{dw_1} = (\hat{y}-y)x_1,...,\dfrac{dL}{dw_n} = (\hat{y}-y)x_n$$. To vectorize it, we have $$\dfrac{dL}{dw} = (\hat{y}-y)x$$ where $$x=\begin{bmatrix}x_1&..&x_n\\\end{bmatrix}^T$$ and $$w=\begin{bmatrix}w_1&..&w_n\\\end{bmatrix}^T$$.


Therefore, for a single sample, we have inferred that $$\dfrac{dL}{dw}^{(j)}=(\hat{y}^{(j)}-y^{(j)})x^{j}$$ and $$$$
